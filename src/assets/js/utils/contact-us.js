...
export const sendNutritionInquiry = async (data) => {
  try {
    const { name, email, mobile, source, message, subject } = data;
    ...
    if (
      !validEmail.match(emailRegex) ||
      validEmail.match(emailRegex)[0] !== validEmail
    ) {
      throw new Error("Invalid Email");
    }
