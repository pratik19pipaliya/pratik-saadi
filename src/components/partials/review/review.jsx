...
function ReviewSection({ book_id }) {
    ...
    const handleSubmit = (book_id) => {
        ...
        axiosInstance.post('/feedback/books', payload)
            .then(result => {
    ...
    const getBookFeedback = () => {
        publicAxiosInstance.get(`/feedback/books?book_id=${book_id}`)
            .then(response => {
                const { data } = response;
            ...
            .catch(error => {
                ...
                Swal.fire({
                    title: 'Error',
                    text: 'Failed to fetch book feedback. Please try again later.',
                    icon: 'error',
                });
            });
    };
